...
const createPathArray = async () => {
  const printDirectoryContents = async (
    directory,
    parentPath = '/',
    result = {
      arr: [],
    ...
    const findClosestLayout = async (directory, isLayout) => {
      // 讀取指定目錄中的檔案列表
      ...
      return await findClosestLayout(parentDirectory)
    }
    ...
    for (const file of files) {
      ...
      if (stat.isDirectory()) {
        ...
        // 尋找最接近的布局檔案並進行路徑格式化
        const closestLayout = await findClosestLayout(directory, isLayout)
        ...
        // 如果未設置名稱，則使用路由路徑作為名稱
        if (!meta.name) meta.name = routePath
        // 建立路由物件
        const item = {
          routePath,
          filePath,
  ...
  const organizeImports = (importString) => {
    /**
     * 將 import 字串進行整理。
     *
  ...
  const importString = organizeImports(result.importString)
...
const createOutput = (arr, initOutputString) => {
  const parentKey = (filePath, obj) => {
    ...
    for (let key of keys) {
      ...
      if (
        filePath.replace('/layout', '').includes(key.replace('/layout', ''))
      ) {
        return key
      }
    }
  ...
  const sortObjectsByFilePathAndIndex = (arr) => {
    /**
     * 根據檔案路徑和索引對物件陣列進行排序。
  ...
  const createComponentName = (filePath) => {
    /**
     * 根據檔案路徑生成組件名稱
     * @param {string} filePath - 檔案路徑
...
console.cuslog = async (data, fileName = 'log') => {
